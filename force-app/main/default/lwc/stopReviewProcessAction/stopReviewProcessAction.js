...
const NEXT_STATUS = 'Completed';
export default class StopReviewProcessAction extends LightningElement {
    @api reviewProcessId;
        ...
        await moveReviewProcessToNextStage({ reviewProcessId: this.recordId, newStatus: NEXT_STATUS })
            .then((data) => {
                showToast(
                    ...
                    'Review Process has been moved to the "Completed" status and hidden from reviewers',
                    'success'
                );
            })
            .catch((error) => {
